...
class Glm4MoeLiteMTPModel(Model):
    """
    GLM-4.7-Flash MTP head: DeepSeek-V3 shape (enorm/hnorm/eh_proj concat into one trunk-style
    decoder layer, shared_head.norm before the shared lm_head), stored as model.layers.47.
    The layer is full-fat: plain MLA attention (no DSA indexer) and the complete 64+1-expert MoE.
    One native draft depth (num_nextn_predict_layers 1); deeper drafts iterate the block on its
    own output, DeepSeek style.

    The MTP borrows the target's embed_tokens and lm_head (like the DeepSeek-V3 / GLM-5.2 MTP);
    its own model.layers.47.embed_tokens and shared_head.head tensors are not loaded.
    """
    ...
    def attach_to(self, target):
        ...
        hidden state. The MTP wants the trunk's POST-final-norm state (the target's final
        RMSNorm output), consumed by hnorm.
        """
